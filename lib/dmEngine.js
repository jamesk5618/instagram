...
function igLogin(acc) {
  // ✅ If password exists → use password login
  if (acc.password && acc.password.trim() !== '') {
    addLog(`[PASSWORD MODE] Logging in @${acc.username}`, null, acc.id);
  ...
  return {
    ok: false,
    error: `No password or session for @${acc.username}`,
    reason: 'no_auth'
  };
}
...
if (!pwd && !sessionFileExists(acc.username)) {
  addLog(`❌ No password or session for @${acc.username}`, campaignId, acc.id, 'error');
  continue;
}
