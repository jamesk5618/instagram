...
// ── Resolve Python executable (cached after first success) ────────
let _resolvedPyCmd = null;

function resolvePyCmd() {
  if (_resolvedPyCmd) return Promise.resolve(_resolvedPyCmd);
  // Override via .env: PYTHON_CMD=/usr/bin/python3
  if (process.env.PYTHON_CMD) {
    _resolvedPyCmd = process.env.PYTHON_CMD;
    return Promise.resolve(_resolvedPyCmd);
  }
  ...
  return new Promise(resolve => {
    ...
    function tryNext() {
      if (idx >= candidates.length) {
        _resolvedPyCmd = candidates[0];
        return resolve(_resolvedPyCmd);
      }
      const cmd = candidates[idx++];
      const p = spawn(cmd, ['--version'], { timeout: 5000 });
      p.on('close', code => { if (code === 0) { _resolvedPyCmd = cmd; resolve(cmd); } else tryNext(); });
      p.on('error', () => tryNext());
    }
    tryNext();
  });
...
function callPython(payload) {
  return resolvePyCmd().then(pyCmd => new Promise((resolve) => {
