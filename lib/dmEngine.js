...
if (sessionFileExists(acc.username)) {
  password = ''; // important → empty triggers session mode
  addLog(`[SESSION MODE] Using session for @${acc.username}`, campaignId, acc.id);
} else {
  addLog(`❌ No session file found for @${acc.username}`, campaignId, acc.id, 'error');
  continue;
}
