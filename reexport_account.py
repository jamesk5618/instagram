#!/usr/bin/env python3
"""
InstaReach v3 — Account-Specific Session Re-Export Tool
For fixing login failures on existing accounts

Usage:
    python reexport_account.py
    
Then upload the generated session_*.json to:
    Hostinger → nodeis/data/sessions/
"""
...
def export_session(username, password):
    ...
    try:
        ...
        if 'challenge' in error_msg or 'checkpoint' in error_msg:
            ...
            print("3. Approve the login from a trusted device")
            print("4. Wait 24 hours")
            print("5. Try again")
            print("="*60)
            ...
            print("    Settings → Security → Two-Factor Authentication → OFF")
            print("  • Then run this script again")
            print("  • After export, re-enable 2FA")
            ...
            print("  • Log in manually first to resolve any pending actions")
...
def main():
    """Main function"""
    print_header("InstaReach v3 — Account Re-Export Tool")
    
    # Check if instagrapi is installed
    if not check_instagrapi_installed():
        install_instagrapi()
    
    print("This tool re-exports your Instagram session to fix login issues.")
    print("Session will be saved as: session_USERNAME.json")
    print()
    ...
    print(f"   Destination: nodeis/data/sessions/{session_file}")
    print()
    print("2. Steps to upload:")
    print("   • Go to Hostinger hPanel")
    print("   • Click: Hosting → File Manager")
    print("   • Navigate to: nodeis/data/sessions/")
    print(f"   • Upload {session_file} into that folder")
    print()
    print("3. Delete old session (if exists):")
    print("   • In File Manager, find old session_*.json")
    ...
    print("   • Click: + Add")
    print()
    print("5. Run test campaign:")
    print("   • Go to: SETUP → Campaigns → Create Campaign")
    print("   • Set: Max Targets = 10 (small test)")
    print("   • Click: ▶ Run")
    print("   • Check: ACTIVITY → Engine Logs")
    print()
    print("="*60)
    print_success("Session exported! Follow the steps above to use it.")
    print("="*60 + "\n")
